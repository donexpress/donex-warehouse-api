...
export const index = async (req: Request, res: Response) => {
    try {
        ...
        const number_of_rows = req.query.number_of_rows ? Number(req.query.number_of_rows) : await AppDataSource.manager.count(Department)
...
export const count = async (req: Request, res: Response) => {
    try {
        const count = await AppDataSource.manager.count(Department)
        res.json({count})
    } catch(e) {
        res.status(500).send(e)
    }
...
export const update = async (req: Request, res: Response) => {
    try {
        const repository = await AppDataSource.getRepository(Department);
