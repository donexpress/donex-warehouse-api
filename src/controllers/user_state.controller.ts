...
import { UserState } from "../entity/UserState.entity";
...
export const index = async (req: Request, res: Response) => {
    try {
        ...
        const states = await AppDataSource.manager.find(UserState, {
            take: number_of_rows,
...
export const show = async (req: Request, res: Response) => {
    try {
        const state = await AppDataSource.manager.findOne(UserState, {
            where: {
...
export const count = async (req: Request, res: Response) => {
    try {
        const count = await AppDataSource.manager.count(UserState)
        res.json({count})
    } catch(e) {
        res.status(500).send(e)
    }
