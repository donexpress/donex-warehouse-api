...
export const listAffiliationState = async (
  current_page: number,
...
export const createAffiliationState = async (affiliation_state_data) => {
  ...
  return await validateContext(AppDataSource, affiliation);
};

export const updateAffiliationState = async (
  id: number,
  affiliation_state_data
) => {
  const repository = await AppDataSource.getRepository(AffiliationState);
